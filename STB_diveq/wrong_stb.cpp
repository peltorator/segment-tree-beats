...
struct DivideSegmentTree {
    ...
    struct Node {
        ...
        long long pushSum;
        long long pushEq;
        // We can't have both pushSum and pushEq because their combination is just one pushEq
    } tree[T];
    ...
    void updateDivideEq(int v, int l, int r, int ql, int qr, int val) {
        ...
        if (ql <= l && r <= qr/* && tree[v].max - tree[v].min <= 1*/) {
            long long k = division(tree[v].max, val);
            /*
            if (division(tree[v].min, val) == k) {
                doPushEq(v, l, r, k);
            } else {
                assert(k * val == tree[v].max);
                doPushSum(v, l, r, k - k * val);
            }
            return;*/
            if (k - tree[v].max == division(tree[v].min, val) - tree[v].min) {
                doPushSum(v, l, r, k - tree[v].max);
                return;
            }
        }
        pushToChildren(v, l, r);
