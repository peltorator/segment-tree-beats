...
struct SqrtSegmentTree {
    ...
    void updateSqrtEq(int v, int l, int r, int ql, int qr) {
        if (qr <= l || r <= ql) {
            return;
        }
        pushToChildren(v, l, r);
        if (ql <= l && r <= qr && tree[v].max - tree[v].min <= 1) {
            long long maxSqrt = sqrt(tree[v].max);
            long long minSqrt = sqrt(tree[v].min);
            if (maxSqrt == minSqrt) {
                doPushEq(v, l, r, maxSqrt);
                return;
            } else {
                doPushSum(v, l, r, maxSqrt - tree[v].max);
                return;
            }
        }
        int mid = (r + l) / 2;
        updateSqrtEq(2 * v, l, mid, ql, qr);
        updateSqrtEq(2 * v + 1, mid, r, ql, qr);
        updateFromChildren(v);
    }

    void updateSqrtEq(int ql, int qr) {
